use std::{cell::{Ref, RefCell, RefMut, UnsafeCell}, mem::MaybeUninit, thread::{self, ThreadId}};
...
/// * Using this struct across threads will panic.
/// * Mixing mutabilty of borrows will panic.
pub struct SingletonLazy<T: 'static, F: FnOnce() -> T> {
...
impl<T: 'static, F: FnOnce() -> T> SingletonLazy<T, F> {
	impl_thread_check!();
	...
	#[inline(always)]
	pub const fn new(initializer: FnOnce() -> T + 'static) -> Self {
		Self {
			inner: OnceCell::new(),
			thread: UnsafeCell::new(None),
			initializer: UnsafeCell::new(Some(Box::new(initializer)))
		}
	}
}